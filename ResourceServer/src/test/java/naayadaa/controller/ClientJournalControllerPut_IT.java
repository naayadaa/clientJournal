...
public class ClientJournalControllerPut_IT extends AbstractIntegrationTest {
   ...
   */
    @Test
    @ExpectedDatabase(value = "classpath:data.xml", assertionMode = DatabaseAssertionMode.NON_STRICT)
    public void updateClient_WithUnauthorisedUser() throws Exception {
        ClientDTO postBody = clientDTOS.get(111);
        postBody.setId(null);
        mockMvc.perform(put("/client-journal-resource/clients/{id}", 1)
    ...
    @Test
    @ExpectedDatabase(value = "classpath:update_expected.xml", assertionMode = DatabaseAssertionMode.NON_STRICT)
    public void updateClient_Test_OK() throws Exception {
        ClientDTO postBody = clientDTOS.get(111);
        postBody.setId(null);

        RequestPostProcessor bearerToken = addBearerToken("test");

        MvcResult result = mockMvc.perform(put("/client-journal-resource/clients/{id}", 1)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsBytes(postBody))
                .with(bearerToken))
    ...
    @Test
    @ExpectedDatabase(value = "classpath:data.xml", assertionMode = DatabaseAssertionMode.NON_STRICT)
    public void updateClient_ValidationException() throws Exception {
        ClientDTO postBody = clientDTOS.get(111);
        postBody.setId(null);
